"""
This is a standalone file to verify
that whether generative method
works for that forecasting problem ?
"""
...
class GEN(nn.Module):
    def __init__(self, encoder, input_len, output_len):
        super(GEN, self).__init__()
        self.encoder = encoder
        self.output_len = output_len
        self.input_len = input_len

        self.generator = nn.Linear(self.input_len, self.output_len, bias=False)

        self.d_model = self.encoder.d_model
        self.d_output = self.encoder.d_output
        self.d_state = self.encoder.d_state
    ...
    def forward(self, x, y=None, **kwargs):
        ...
        return: B H output_length
        """

        B, L, H = x.shape
        # default_state = self.encoder.default_state(B)
        # _, next_state = self.encoder(x, state=default_state)

        # 生成模型
        y_hat = self.generator(x.permute(0, 2, 1)).transpose(1, 2)
        y_hat = torch.cat([x, y_hat], dim=1)
        origin, _ = self.encoder(y_hat)
        y = torch.cat([x, y], dim=1)
        target, _ = self.encoder(y)
...
class SequenceLightningModule(pl.LightningModule):
    ...
    def setup(self, stage=None):
        ...
        decoder_cfg = utils.to_list(self.hparams.model.pop("decoder", None)
                                    ) + utils.to_list(self.hparams.decoder)

        # Instantiate model
        self.extractor = utils.instantiate(registry.model, self.hparams.model)
        self.de_extractor = utils.instantiate(registry.model, self.hparams.model)
        self.model = GEN(encoder=self.extractor,
                         input_len=self.dataset.l_input,
                         output_len=self.dataset.l_output)
    ...
    def forward(self, batch):
        ...
        y_hidden, _ = self.encoder(y)  # only use in autogressive view
        origin, target = self.model(x, **w, y=y_hidden, state=self._state)
        self._state = None
        y_reserve, w = self.decoder(origin[:, -y.shape[1]:, :], state=None, **z)
        return origin, target, y_reserve
    ...
    def _shared_step(self, batch, batch_idx, prefix="train", epoch_end=False):

        self._process_state(batch, batch_idx, train=(prefix == "train"))

        _, y, *xyz = batch

        origin, target, y_reserve = self.forward(batch)

        self.eval_y_reserve = y_reserve
        self.eval_y = y
        self.eval_prefix = prefix

        # Loss
        if prefix == 'train':
            loss = self.loss(origin, target) + self.loss(y_reserve, y)
        else:
            loss = self.loss_val(y_reserve, y)

        # Metrics
        metrics = self.metrics(y_reserve, y)
        metrics["loss"] = loss
        metrics = {f"{prefix}/{k}": v for k, v in metrics.items()}

        # Calculate torchmetrics: these are accumulated and logged at the end of epochs
        self.task.torchmetrics(y_reserve, y, prefix)
    ...
    def plt2verify(self):
        ...
        y_hat = self.eval_y_reserve[-1, :, 0]
        y = self.eval_y[-1, :, 0]

        ax.plot(y_hat.detach().cpu().numpy(), linewidth=1.0, label='y_hat')
        ax.plot(y.detach().cpu().numpy(), linewidth=1.0, label='y')

        plt.legend()
        if os.path.isdir(r'D:\WSQ\OneDrive\instance\S4\verify\gen'):
            plt.savefig(fr'D:\WSQ\OneDrive\instance\S4\verify\gen\gen_{self.eval_prefix}.pdf', bbox_inches='tight')
        plt.show()
        plt.close()
