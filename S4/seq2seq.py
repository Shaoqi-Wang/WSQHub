"""
This is a standalone file to verify which kind of forecasting method better, autoregressive or plain
"""
...
class Seq2Seq(nn.Module):
    def __init__(self, encoder, decoder, output_len: int):
    ...
    def forward(self, x, y=None, **kwargs):
        ...
        """
        B, L, H = x.shape
        default_state = self.decoder.default_state(B)
        ...
        if self.training:
            ...
            decoder_input = torch.concat([torch.zeros([*y.shape[:2], 1]).to(x), y[:, :, :-1]], dim=-1)
            outputs, next_state = self.decoder(decoder_input, state=next_state)

        # 验证阶段
        else:
            # 自回归输出
            decoder_input = torch.zeros([x.shape[0], x.shape[2]]).to(x)
            outputs = []
            for _ in range(self.output_len):
                decoder_input, next_state = self.decoder.step(decoder_input, next_state)
...
class SequenceLightningModule(pl.LightningModule):
    ...
    def setup(self, stage=None):
        ...
        self.model = Seq2Seq(encoder=self.extractor,
                             decoder=self.de_extractor,
    ...
    def forward(self, batch):
        ...
        x, state = self.model(x, **w, y=y_hidden, state=self._state)
        self._state = state
    ...
    def _shared_step(self, batch, batch_idx, prefix="train"):
        ...
        self.eval_y_reserve = self.dataset.dataset_train.scaler.inverse_transform(x)
        self.eval_y = self.dataset.dataset_train.scaler.inverse_transform(y)
