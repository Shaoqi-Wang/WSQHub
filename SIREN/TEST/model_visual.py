...
class TESTmodel(nn.Module):
    def __init__(self):
        ...
        output_layer = [BatchLinear(hidden_dim, output_dim)]

        self.net = MetaSequential(*(input_layer + hidden_layers + output_layer))
    ...
    def forward(self, coords, params=None, **kwargs):
        ...
        for i, layer in enumerate(self.net):
            if isinstance(layer, BatchLinear) or isinstance(layer, Sine):
                ...
                input = layer(input, params=get_subdict(params, f'net.{i}'))
        return input, grads_output


class EVALmodel(TESTmodel):
    def __init__(self):
...
if __name__ == '__main__':
    ...
    coords = torch.rand(batch_size, input_dim, requires_grad=True) * 2 - 1

    model_output, layer_outpus_grads = model(coords)

    # 计算概率密度（直方图）

    grad_1 = layer_outpus_grads['6_grad'].detach().reshape(-1).numpy()

    hist, bin_edges = np.histogram(grad_1, bins=256, density=True)
