...
class Seq2Seq(nn.Module):
    ...
    def forward(self, x, u=None, **kwargs):
        ...
        """
        B, L, H = x.shape
        origin_state, _ = self.encoder(x)

        outputs, rect_loss = self.decoder(origin_state=origin_state, u=u)

        return outputs, rect_loss
...
class SequenceLightningModule(pl.LightningModule):
    ...
    def setup(self, stage=None):
        ...
        self.hparams.model['_name_'] = 'de_model'
    ...
    def forward(self, batch):
        ...
        x, w = self.encoder(x, **z)  # 对于ssformer而言，encoder不一定是nn.identity的
        y_encoded, _ = self.encoder(y, **z)  # 对于ssformer而言，encoder不一定是nn.identity的
        x, reconstruct_loss = self.model(x, **w, u=y_encoded)
        x, w = self.decoder(x, **z)
        return x, y, reconstruct_loss
