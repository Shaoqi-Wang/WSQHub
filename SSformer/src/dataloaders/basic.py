...
from src.dataloaders.utils.cifar_augmentations import Cutout
...
def mnist_lambda_1(d_input, L, x):
    return x.view(d_input, L).t()


def mnist_lambda_2(permutation, x):
...
class MNIST(SequenceDataset):
    ...
    def setup(self):
        ...
        lambda_1 = partial(mnist_lambda_1, self.d_input, self.L)

        transform_list = [
            torchvision.transforms.ToTensor(),
            torchvision.transforms.Lambda(lambda_1),
        ]  # (L, d_input)
        if self.permute:
            ...
            lambda_2 = partial(mnist_lambda_2, permutation)

            transform_list.append(
                torchvision.transforms.Lambda(lambda_2)
            )
    ...
    def setup(self):
        ...
        if self.rescale:
            ...
            self.dataset_train.data = self.dataset_train.data.reshape(
                (self.dataset_train.data.shape[0], 32 // self.rescale, self.rescale, 32 // self.rescale, self.rescale, 3)).max(4).max(2).astype(np.uint8)
            self.dataset_test.data = self.dataset_test.data.reshape((self.dataset_test.data.shape[0], 32 // self.rescale, self.rescale, 32 // self.rescale, self.rescale, 3)).max(
                4).max(2).astype(np.uint8)
