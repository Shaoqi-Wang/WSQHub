...
def basis(method, N, vals, c=0.0, truncate_measure=True):
    ...
    if method == 'legt':
        ...
        #  勒让德多项式在n为偶数时是偶函数，反之是奇函数，此处坐标应该是2e^{-x}-1，它先反向，而后再(-1)^{n}，不知道在想什么
        _vals = np.exp(-vals)
...
def plot(T, dt, N, freq):
    ...
    offset = 0.0
    plt.plot(vals, u.cpu() + offset, 'k', linewidth=1.0, label='whitesignal')
...
if __name__ == '__main__':
    # plot(T=3, dt=1e-3, N=64, freq=20.0)

    # Visualize HiPPO online reconstruction

    ani = animate_hippo(
        'legt',  # Try 'legt' or 'fourier'
