...
class SequenceResidualBlock(SequenceModule):
    def __init__(
        ...
        if layer['_name_'] in ['s4', 's4attention']:
            this_layer = layer.copy()
            this_layer['_name_'] = 's4'
        else:
            this_layer = layer
        ...
        if ssfeedback:
            self.layer = utils.instantiate(registry.layer, this_layer, d_input, ssfeedback=ssfeedback)
        else:
            self.layer = utils.instantiate(registry.layer, this_layer, d_input)
...
class SequenceDecoderBlock(SequenceModule):
    def __init__(
            self,
            d_input,
            i_layer=None,  # Only needs to be passed into certain residuals like Decay
    ...
    def forward(self, origin_state, u=None, previous_decode=None, **kwargs):

        pd = previous_decode
        # Pre-norm
        if self.norm is not None and self.prenorm and u is not None: u = self.norm(u)

        # Black box layer
        y, reconstruct_loss = self.layer(u=u, origin_state=origin_state, previous_decode=pd, **kwargs)
