...
from src.models.sequence.block import SequenceResidualBlock, SequenceDecoderBlock
...
class SequenceModel(SequenceModule):
    def __init__(
        ...
        # Duplicate layers
        layers = layer * (n_layers - 1) * n_repeat
        ...
        if ssfeedback:
            block = SequenceResidualBlock(d, l + 1, prenorm=prenorm, dropout=dropout, tie_dropout=tie_dropout, transposed=transposed, layer=layer, residual=residual, norm=norm,
                                          pool=pool_cfg, ssfeedback=ssfeedback)
            _layers.append(block)
            d = block.d_output
    ...
    def forward(self, inputs, *args, state=None, **kwargs):
        ...
        if self.ssfeedback:
            return outputs, 0  # 输出为最后一层的状态(B H N)
...
class SequenceDecoderModel(SequenceModule):
    def __init__(
            self,
            d_model,  # Resize input (useful for deep models with residuals)
            n_layers=1,  # Number of layers
        ...
        for l, layer in enumerate(layers):
            # Pool at the end of every n_repeat blocks
            pool_cfg = pool if (l + 1) % n_repeat == 0 else None
            block = SequenceDecoderBlock(d, l + 1, prenorm=prenorm, dropout=dropout, tie_dropout=tie_dropout, transposed=transposed, layer=layer, residual=residual, norm=norm,
                                         pool=pool_cfg)
            _layers.append(block)
            d = block.d_output
    ...
    def forward(self, origin_state, *args, u=None, **kwargs):
        """ Inputs assumed to be (batch, sequence, dim) """
        if self.transposed: inputs = rearrange(u, 'b ... d -> b d ...')
        u = self.drop(u)

        # Apply layers
        pd = None
        rect_loss = torch.tensor([0]).to(origin_state)
        assert len(self.layers) >= 1
        for layer in self.layers:
            outputs, reconstruct_loss = layer(origin_state=origin_state, u=u, previous_decode=pd, *args, **kwargs)
            rect_loss += reconstruct_loss
            pd = outputs
        if self.norm is not None: outputs = self.norm(outputs)

        if self.transposed: outputs = rearrange(outputs, 'b d ... -> b ... d')

        return outputs, rect_loss.to(outputs)
