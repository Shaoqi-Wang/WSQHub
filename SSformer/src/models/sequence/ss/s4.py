...
from src.models.sequence import SSAttention
...
class S4(nn.Module):
    def __init__(
        ...
        # SSM Kernel
        self.kernel = SSKernel(self.H, N=self.N, L=self.L, channels=channels, verbose=verbose, ssfeedback=ssfeedback, **kernel_args)
    ...
    def forward(self, u, state=None, rate=1.0, lengths=None, **kwargs):  # absorbs return_output and transformer src mask
        ...
        # 在状态反馈中，C就是N，不需要rearrange
        if self.ssfeedback:
            y = y
            return y.transpose(-2, -3), 0  # 返回 (B H N L)
        else:
...
class S4former(nn.Module):
    def __init__(
        ...
        # SSM Kernel
        self.kernel = SSAttention(self.H, N=self.N, L=self.L, channels=channels, verbose=verbose, **kernel_args)
    ...
    def forward(self, origin_state, u=None, previous_decode=None, **kwargs):  # absorbs return_output and transformer src mask
        """
        u: (B H L) if self.transposed else (B L H)
        state: (H N) never needed unless you know what you're doing
        ...
        if u is not None:
            ...
            raise ValueError('没有指定L')

        L_kernel = L

        # Compute SS Kernel
        y, reconstruct_loss = self.kernel(origin_state=origin_state, L=L_kernel, u=u, previous_decode=pd)  # (C H L) (B C H L)
        # Reshape to flatten channels
        y = rearrange(y, '... c h l -> ... (c h) l')
